...
import { auditToday, isLocalAuditMode } from "./local-audit";
...
function isoDaysAgo(days: number): string {
  const date = isLocalAuditMode() ? new Date(`${auditToday()}T00:00:00Z`) : new Date();
  date.setDate(date.getDate() - days);
  return date.toISOString().slice(0, 10);
}

function uiToday(): string {
  return isLocalAuditMode() ? auditToday() : new Date().toISOString().slice(0, 10);
}
...
export default function App() {
  ...
  const [chartEnvironment, setChartEnvironment] = useState("production");
...
function RetainedRecordModal({ event, farms, houses, flocks, onClose, onSubmit }: { event: ReliabilityEvent; farms: Farm[]; houses: House[]; flocks: Flock[]; onClose: () => void; onSubmit: (body: Record<string, unknown>) => Promise<boolean> }) {
  ...
  const [eventDate, setEventDate] = useState(uiToday());
  const [note, setNote] = useState("");
  const [reason, setReason] = useState("");
  const [error, setError] = useState("");
  const [busy, setBusy] = useState(false);
...
function MessageDiagnosticsView({ preview, events, onPage }: { preview: AmbientPreview | null; events: ReliabilityEvent[]; onPage?: (page: number) => void }) {
  ...
  const stageLabel = (value: string): string => ({ lease: "租約", source: "來源", prefilter: "預篩", ai: "AI", validation: "驗證", reconcile: "對帳", candidateWrite: "候選寫入", bufferConsume: "緩衝消費", delivery: "傳送" } as Record<string, string>)[value] ?? value;
  const stageSummary = (stages: AmbientPreview["recentDigestRuns"][number]["stages"]): string => Object.entries(stages).map(([key, value]) => `${stageLabel(key)}:${value}`).join(" · ");
  return <section className="page">
    ...
    <section className="panel"><PanelTitle title="最近 Ambient 排程執行" /><p className="muted">只顯示既有 durable metadata 的短編號、階段、終態與保存期限；不顯示 LINE 原文、使用者或群組識別資料。</p>{!preview.observabilityAvailable ? <div className="notice">目前無法讀取排程執行 metadata；既有訊息診斷仍可使用。</div> : <>{preview.recentDigestInvocations.length ? <DataTable headers={["Invocation", "排程", "終態", "重試次數", "群組／Run"]}>{preview.recentDigestInvocations.map((invocation) => <tr key={invocation.invocationIdShort}><td><code>{invocation.invocationIdShort}</code></td><td>{taipeiTime(invocation.scheduledFor)}</td><td><StatusPill tone={invocation.invocationStatus === "completed" ? "good" : "warn"}>{invocation.invocationStatus}</StatusPill></td><td>{invocation.attemptCount}</td><td>{invocation.groupsAfterCleanup} 群組／{invocation.perGroupRunsCreated} Run</td></tr>)}</DataTable> : <div className="empty">目前沒有可與本組織安全關聯的排程 invocation。</div>}{preview.recentDigestRuns.length ? <DataTable headers={["Run", "Invocation", "排程", "終態／重試", "階段", "問題"]}>{preview.recentDigestRuns.map((run) => <tr key={run.runIdShort}><td><code>{run.runIdShort}</code></td><td><code>{run.invocationIdShort ?? "—"}</code></td><td>{taipeiTime(run.scheduledFor)}</td><td><StatusPill tone={run.runStatus === "completed" ? "good" : "warn"}>{run.runStatus}／{run.retryState}</StatusPill></td><td><small>{stageSummary(run.stages)}</small></td><td>{run.errorStage ?? "—"}{run.errorClass ? ` · ${run.errorClass}` : ""}</td></tr>)}</DataTable> : <div className="empty">目前沒有可顯示的排程 Run。</div>}<p className="technical-meta">執行截止時間：尚未由既有 metadata 記錄；保存期限：以各 invocation／Run 的 retention expiry 為準。</p></>}</section>
...
function FarmsView({ farms, initialFarmId, onCreate, onUpdate, onRecord }: { farms: Farm[]; initialFarmId: string | null; onCreate: (body: Record<string, unknown>) => MutationResult; onUpdate: (id: string, body: Record<string, unknown>, structural: boolean) => MutationResult; onRecord: (farmId: string) => void }) {
...
function SettingsView({ farms, organization }: { farms: Farm[]; organization: { id: string; name: string; active: boolean } | null }) { const localAudit = isLocalAuditMode(); return <section className="page"><div className="panel settings"><PanelTitle title="系統設定" /><div className="setting-row"><span>LINE 助理</span><strong>{localAudit ? "本地稽核虛擬助理" : "部署版本設定的 LINE 助理"}</strong></div><div className="setting-row"><span>後端服務</span><strong>{localAudit ? "local-audit-memory-adapter" : "部署版本設定的後端服務"}</strong></div><div className="setting-row"><span>資料庫</span><strong>{localAudit ? "記憶體虛擬資料（重新載入即重置）" : "部署版本設定的資料來源"}</strong></div><div className="setting-row technical-row"><span>AI 模型</span><strong>{localAudit ? "synthetic-audit-fixture" : "部署版本設定的 AI 模型"}</strong></div><div className="setting-row"><span>協會組織</span><strong>{organization?.name ?? "—"}</strong></div><div className="setting-row"><span>雞場範圍</span><strong>{farms.length} 個雞場（含測試）</strong></div><div className="setting-row"><span>LINE 訊息重送</span><strong>{localAudit ? "本地模式不連 LINE" : "需要到 LINE Developers 網頁確認"}</strong></div><div className="notice">{localAudit ? "這是 100% 虛擬資料；本地操作只存在目前分頁記憶體，不會寫入正式環境。" : "目前程式沒有可驗證的人工重送設定結果，也不會自行猜測或修改外部設定。編修會沿用目前登入狀態，並保留完整變更紀錄供查閱。"}</div></div></section>; }
