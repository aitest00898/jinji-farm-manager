...
try {
  ...
  rows("INSERT INTO abnormal_events (id, organization_id, farm_id, occurred_at, occurred_date, reported_at, raw_text, source, source_event_id, taxonomy_id, family, canonical_type, subtype, extent, source_channel) VALUES ('abnormal-a16-optional-evidence', 'org-taxonomy-rehearsal', 'farm-taxonomy-rehearsal', '2026-09-08T01:00:00.000Z', '2026-09-08', '2026-09-08T01:00:00.000Z', 'synthetic attack without evidence', 'web', 'source-abnormal-a16-optional-evidence', 'A16', 'operational_observation', 'observation', 'attack', 'small', 'web');");
  assert.deepEqual(rows("SELECT taxonomy_id, subtype, evidence FROM abnormal_events WHERE id = 'abnormal-a16-optional-evidence'"), [{ taxonomy_id: "A16", subtype: "attack", evidence: null }]);
  ...
  expectSqlFailure("INSERT INTO operational_actions (id, organization_id, taxonomy_id, family, canonical_type, subtype, occurred_at, farm_id, source_channel, raw_text, client_operation_id) VALUES ('bad-o7', 'org-taxonomy-rehearsal', 'O7', 'operational_action', 'action', 'disinfection', '2026-09-08T01:00:00.000Z', 'farm-taxonomy-rehearsal', 'web', 'bad O7', 'bad-o7-client');", "CHECK");
