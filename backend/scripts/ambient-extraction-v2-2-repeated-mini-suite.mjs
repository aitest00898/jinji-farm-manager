...
const MAX_PROVIDER_CALLS = 9;
const projectRoot = process.cwd();
const args = process.argv.slice(2);
const markerPrefix = "AMBIENT_V2_2_REAL_MINI_SUITE_SAFE_JSON=";
...
async function readLedger(path) {
  ...
  try {
    ...
    throw new Error("V2_2_MINI_LEDGER_READ_FAILURE");
  }
  const records = [];
...
function evaluateWrapper(reconstructed, marker, processResult, invalidLineCount) {
  const reasons = [];
  if (invalidLineCount > 0) reasons.push("LEDGER_CORRUPT");
  ...
  if (marker?.providerCalls !== undefined && marker.providerCalls !== MAX_PROVIDER_CALLS) reasons.push("PROVIDER_CALL_COUNT_MISMATCH");
  if (marker?.summary?.totalProviderCalls !== undefined && marker.summary.totalProviderCalls !== MAX_PROVIDER_CALLS) reasons.push("SUMMARY_CALL_COUNT_MISMATCH");
  return {
...
async function run() {
  ...
  const ledgerPath = process.env.AMBIENT_V2_2_REAL_MINI_SUITE_LEDGER_PATH
    || join(projectRoot, "forensics", "runtime", `ambient-extraction-v2-2-repeated-mini-suite-${experimentId}.jsonl`);
  const existing = await readLedger(ledgerPath);
  if (existing.invalidLineCount > 0) {
    fail("V2_2_MINI_LEDGER_CORRUPT_BEFORE_RUN");
    return;
  }
  if (existing.records.length > 0) {
    fail("V2_2_MINI_LEDGER_NOT_EMPTY");
    return;
  }
  ...
  const child = spawn(
    ...
    ["run", "src/ambient-extraction-v2-2-real-mini-suite.test.ts", "--reporter=dot"],
    {
      cwd: projectRoot,
      stdio: ["ignore", "pipe", "pipe"],
      env: buildSafeAmbientChildEnvironment({
        AMBIENT_V2_2_REAL_MINI_SUITE: "1",
        AMBIENT_V2_2_REAL_MINI_SUITE_ACCOUNT_ID: account.value,
        AMBIENT_V2_2_REAL_MINI_SUITE_LEDGER_PATH: ledgerPath,
        AMBIENT_V2_2_REAL_MINI_SUITE_EXPERIMENT_ID: experimentId,
        AMBIENT_V2_2_REAL_MINI_SUITE_MATRIX_RUN_ID: matrixRunId,
        AMBIENT_V2_2_REAL_MINI_SUITE_MODEL: MODEL,
        AMBIENT_V2_2_REAL_MINI_SUITE_MAX_CALLS: String(MAX_PROVIDER_CALLS),
        AMBIENT_V2_2_REAL_MINI_SUITE_ENDPOINT: endpoint,
      }),
    },
  );
  let childOutput = "";
  ...
  try {
    ...
    const reconstructed = reconstruct(after.records, experimentId, matrixRunId, processResult, Boolean(marker));
    reconstructed.invalidLineCount = after.invalidLineCount;
    const wrapper = evaluateWrapper(reconstructed, marker, processResult, after.invalidLineCount);
    console.log(`V2_2_MINI_LEDGER_PATH_SAFE=${ledgerPath.replace(projectRoot, "<PROJECT>")}`);
    console.log(`AMBIENT_V2_2_MINI_RECONSTRUCTED_SAFE_JSON=${JSON.stringify(reconstructed)}`);
    if (marker) console.log(`${markerPrefix}${JSON.stringify(marker)}`);
    console.log(`AMBIENT_V2_2_MINI_MARKER_STATUS=${wrapper.markerStatus}`);
    console.log(`AMBIENT_V2_2_MINI_WRAPPER_STATUS=${wrapper.pass ? "PASS" : "FAIL"}`);
    if (!wrapper.pass) {
      console.log(`AMBIENT_V2_2_MINI_WRAPPER_FAILURE_CLASSES=${wrapper.reasons.join(",")}`);
      process.exitCode = 2;
      return;
    }
    if (marker && marker.summary?.status !== "PASS_STABILITY") {
      process.exitCode = 2;
    }
  } catch {
    console.error("V2_2_MINI_LEDGER_READ_FAILURE");
    process.exitCode = 2;
  }
}
