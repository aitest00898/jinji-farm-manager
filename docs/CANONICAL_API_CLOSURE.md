...
| Canonical read | `GET /api/records` | active Web session | optional `environment=test`, `farmId`, bounded `limit` | organization + farm environment filter | union read bridge over four authorities | `{ records, lifecycleSummaries, environment }`; each summary includes derived O3 shipment projection, O6 lab status, and deterministic O5 feed estimate |
| One-water lifecycle/readiness read | `GET /api/lifecycle` | active Web session | optional `environment=test`, `farmId`, `houseId` | organization/environment scope plus read-time effective lineage projection | derived from O1/O3/O7/O9, flock master data, and effective stock; no write | `{ lifecycleSummaries, environment }`; each house includes O3 partial/final/reversed readback, O6 pending/overdue status, and deterministic feed estimate |
...
FEED_ESTIMATION_5Y_15_CYCLES = COMPLETE (same-house, read-time deterministic; minimum 3 eligible closed cycles; no O5 write)
O3_SHIPMENT_OPERATIONAL_SEMANTICS = COMPLETE (same effective O1/O3/O9 stock replay; no stored final flag; no second authority)
